...
function:
            Hardware reset
*******************************************************************************/
static void OLED_Reset_wiring(void)
{
    OLED_RST_1;
    Driver_Delay_ms_wiring(100);
    OLED_RST_0;
    Driver_Delay_ms_wiring(100);
    OLED_RST_1;
    Driver_Delay_ms_wiring(100);
...
*******************************************************************************/
void OLED_WriteReg_wiring(uint8_t Reg)
{
#if USE_SPI_4W
    OLED_DC_0;
    OLED_CS_0;
    ...
    OLED_CS_1;
#elif USE_IIC
    I2C_Write_Byte_wiring(Reg,IIC_CMD);
#endif
}

void OLED_WriteData_wiring(uint8_t Data)
{
#if USE_SPI_4W
    OLED_DC_1;
    OLED_CS_0;
    ...
    OLED_CS_1;
#elif USE_IIC
    I2C_Write_Byte_wiring(Data,IIC_RAM);
#endif
}
...
*******************************************************************************/
static void OLED_InitReg_wiring(void)
{
    OLED_WriteReg_wiring(0xae);//--turn off oled panel

    OLED_WriteReg_wiring(0x15);    //   set column address
    OLED_WriteReg_wiring(0x00);    //  start column   0
    OLED_WriteReg_wiring(0x7f);    //  end column   127

    OLED_WriteReg_wiring(0x75);    //   set row address
    OLED_WriteReg_wiring(0x00);    //  start row   0
    OLED_WriteReg_wiring(0x7f);    //  end row   127

    OLED_WriteReg_wiring(0x81);  // set contrast control
    OLED_WriteReg_wiring(0x80);

    OLED_WriteReg_wiring(0xa0);    // gment remap
    OLED_WriteReg_wiring(0x51);   //51

    OLED_WriteReg_wiring(0xa1);  // start line
    OLED_WriteReg_wiring(0x00);

    OLED_WriteReg_wiring(0xa2);  // display offset
    OLED_WriteReg_wiring(0x00);

    OLED_WriteReg_wiring(0xa4);    // rmal display
    OLED_WriteReg_wiring(0xa8);    // set multiplex ratio
    OLED_WriteReg_wiring(0x7f);

    OLED_WriteReg_wiring(0xb1);  // set phase leghth
    OLED_WriteReg_wiring(0xf1);

    OLED_WriteReg_wiring(0xb3);  // set dclk
    OLED_WriteReg_wiring(0x00);  //80Hz:0xc1 90Hz:0xe1   100Hz:0x00   110Hz:0x30 120Hz:0x50   130Hz:0x70     01

    OLED_WriteReg_wiring(0xab);  //
    OLED_WriteReg_wiring(0x01);  //

    OLED_WriteReg_wiring(0xb6);  // set phase leghth
    OLED_WriteReg_wiring(0x0f);

    OLED_WriteReg_wiring(0xbe);
    OLED_WriteReg_wiring(0x0f);

    OLED_WriteReg_wiring(0xbc);
    OLED_WriteReg_wiring(0x08);

    OLED_WriteReg_wiring(0xd5);
    OLED_WriteReg_wiring(0x62);

    OLED_WriteReg_wiring(0xfd);
    OLED_WriteReg_wiring(0x12);
...
********************************************************************************/
void OLED_SetGramScanWay_wiring(OLED_SCAN_DIR Scan_dir)
{
...
function:
            initialization
********************************************************************************/
void OLED_Init_wiring(OLED_SCAN_DIR OLED_ScanDir)
{
    //Hardware reset
    OLED_Reset_wiring();
    ...
    OLED_SetGramScanWay_wiring(OLED_ScanDir );
    Driver_Delay_ms_wiring(200);

    //Turn on the OLED display
    OLED_WriteReg_wiring(0xAF);
}
...
********************************************************************************/
void OLED_SetCursor_wiring(POINT Xpoint, POINT Ypoint)
{
    if((Xpoint > sOLED_DIS.OLED_Dis_Column) || (Ypoint > sOLED_DIS.OLED_Dis_Page))
        return;

    OLED_WriteReg_wiring(0x15);
    OLED_WriteReg_wiring(Xpoint);
    OLED_WriteReg_wiring(Xpoint);

    OLED_WriteReg_wiring(0x75);
    OLED_WriteReg_wiring(Ypoint);
    OLED_WriteReg_wiring(Ypoint);
}
...
********************************************************************************/
void OLED_SetWindow_wiring(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend)
{
    ...
    OLED_WriteReg_wiring(0x15);
    OLED_WriteReg_wiring(Xstart);
    OLED_WriteReg_wiring(Xend - 1);

    OLED_WriteReg_wiring(0x75);
    OLED_WriteReg_wiring(Ystart);
    OLED_WriteReg_wiring(Yend - 1);
}
...
void OLED_SetColor_wiring(POINT Xpoint, POINT Ypoint, COLOR Color)
{
    if(Xpoint > sOLED_DIS.OLED_Dis_Column || Ypoint > sOLED_DIS.OLED_Dis_Page) {
        return;
    }
...
function:
            Clear screen
********************************************************************************/
void OLED_Clear_wiring(COLOR Color)
{
    unsigned int i,m;
...
function:   Update all memory to LCD
********************************************************************************/
void OLED_Display_e_wiring(void)
{
    uint16_t page, Column;
    COLOR *pBuf = (COLOR *)Buffer;
...
function:
            Clear Window
********************************************************************************/
void OLED_ClearWindow_wiring(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend, COLOR Color)
{
...
********************************************************************************/
void OLED_DisWindow_wiring(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend)
{
